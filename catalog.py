...
def itemsJSON(category_name):
    ...
    items = session.query(Item).filter_by(category_id=category.id).order_by(Item.name.desc())   
    return jsonify(Item=[i.serialize for i in items])
    
@app.route('/catalog/<path:category_name>/<path:item_name>/JSON')
def itemsDetailsJSON(category_name, item_name):
    category = session.query(Category).filter_by(name=category_name).one()
    item = session.query(Item).join(Category).filter(Category.id==category.id).filter(Item.name==item_name).one()
...
def showCatalog():
    ...
    #items = session.query(Item).order_by(add_date.desc()).limit(num_Categories)
    #Add code to set based on login
...
def showItems(category_name):    
    ...
    #Add code to set based on login
    loggedIn = logIn()
    return render_template('category.html', categories = categories, category = category, count = num_items, items = items, loggedIn = loggedIn)
...
def showItemsDetails(category_name, item_name):    
    ...
    return render_template('item.html', categories = categories, category = category, item = item, loggedIn = loggedIn)
...
def newItem():   
    ...
    if request.method == 'POST':
        #print("item cat :  ",request.form['itemCategory'])
        #('item cat :  ', u'6')
        newItem = Item(name=request.form['name'], description=request.form['description'], price=request.form['price'], category_id=request.form['itemCategory'], add_date=datetime.datetime.now())
        session.add(newItem)
        session.commit()
    ...
    if request.method == 'POST':    
        if request.form['name']:
            if request.form['name'] != item_name:
                if session.query(Item).filter(Item.name==request.form['name']).one():
                    flash("Item not edited. An item by the desired new name already exists.")
                    return redirect(url_for('showCatalog'))
            ...
            flash("No item has been edited.")
            return redirect(url_for('showCatalog'))
    else:
        return render_template('editItem.html',categories = categories, item = editIt)
...
def deleteItem(item_name):    
    ...
    #Adding flash message
    return render_template('deleteItem.html', categories = categories, item = deleteIt, loggedIn = loggedIn)
